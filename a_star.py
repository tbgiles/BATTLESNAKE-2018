...
def position_passable(game_grid, position):
...
def get_neighbors(game_grid, position):
    (x, y) = position
    return [item for item in [(x-1, y), (x+1, y), (x, y-1), (x, y+1)] if position_passable(game_grid, item)]
...
def a_star(game_grid, start, end, heuristic='default'):
    ...
    if not position_passable(game_grid, end):
    ...
    while not frontier.empty():
        ...
        for neighbor in get_neighbors(game_grid, current):
